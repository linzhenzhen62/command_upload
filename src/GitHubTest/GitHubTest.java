...
import GitHubTest.Util;


public class GitHubTest {
	private WebDriver driver;
	private String baseUrl; // baseUrl of RightEye
	String[] TestName;
...
//		driver.manage().timeouts().implicitlyWait(Util.WAIT_TIME, TimeUnit.SECONDS);
		...
		public void verifyTestName() throws Exception{
			...
			for(int i=0;i<TestName.length;i++){
				TestName[i] = TestNameArray.get(i); // all the test names: TestName[0]=Autism
			}
		}
	...
	public void verifyCircuitSmoothPursuit() throws ParseException{
			...
			for(int i=0;i<TestName.length;i++){
				if(TestName[i].equals("Circular Smooth Pursuit")){
					TestPosition.add(i+1); //i is the test number (here i=1) CSP is the second test
				}
			}
			
			int index = 0;
			
			while(index<TestPosition.size()){
				int CSPnumber = TestPosition.get(index)+4;
				for(int col=2;col<=6;col+=2){
					
					for(int row=4;row<=6;row++){
						value = driver.findElement(By.xpath("//*[@id='content']/div/div/div/div[3]/div["+CSPnumber+"]/div/div[4]/table/tbody/tr["+row+"]/td["+col+"]")).getText();
	...
	public void verifyHorizontalSmoothPursuit() throws ParseException{
			...
			for(int i=0;i<TestName.length;i++){
				if(TestName[i].equals("Horizontal Smooth Pursuit")){
					TestPosition.add(i+1); //i is the test number (here i=1) CSP is the second test
				}
			}
			...
			int index = 0;
			while(index<TestPosition.size()){
				int HSPnumber = TestPosition.get(index)+4;
				for(int col=2;col<=6;col+=2){
					for(int row=4;row<=6;row++){
						value = driver.findElement(By.xpath("//*[@id='content']/div/div/div/div[3]/div["+HSPnumber+"]/div/div[3]/table/tbody/tr["+row+"]/td["+col+"]")).getText();
						NumberFormat f = NumberFormat.getNumberInstance();
						Number num = f.parse(value);
						value = num.toString();
						val = Double.parseDouble(value);
	...
	public void verifyVerticalSmoothPursuit() throws ParseException{
			...
			for(int i=0;i<TestName.length;i++){
				if(TestName[i].equals("Vertical Smooth Pursuit")){
					TestPosition.add(i+1); //i is the test number (here i=1) CSP is the second test
				}
			}
			...
			int index = 0;
			while(index<TestPosition.size()){
				int VSPnumber = TestPosition.get(index)+4;
				for(int col=2;col<=6;col+=2){
					for(int row=4;row<=6;row++){
						value = driver.findElement(By.xpath("//*[@id='content']/div/div/div/div[3]/div["+VSPnumber+"]/div/div[3]/table/tbody/tr["+row+"]/td["+col+"]")).getText();
						NumberFormat f = NumberFormat.getNumberInstance();
						Number num = f.parse(value);
						value = num.toString();
						val = Double.parseDouble(value);
	...
	public void verifyChoiceReactionTime(){
			...
			for(int i=0;i<TestName.length;i++){
				if(TestName[i].equals("Choice Reaction Time")){
					TestPosition.add(i+1); //i is the test number (here i=1) CSP is the second test
				}
			}
			...
			int index = 0;
			while(index<TestPosition.size()){
				int CRTnumber = TestPosition.get(index)+4;
				
				for(int row=3;row<=6;row++){
					value = driver.findElement(By.xpath("//*[@id='content']/div/div/div/div[3]/div["+CRTnumber+"]/div/div[2]/table/tbody/tr["+row+"]/td[2]")).getText();
		...
		public void verifyDiscriminateReactionTime(){
				...
				for(int i=0;i<TestName.length;i++){
					if(TestName[i].equals("Discriminate Reaction Time")){
						TestPosition.add(i+1); //i is the test number (here i=1) CSP is the second test
					}
				}
				...
				int index = 0;
				while(index<TestPosition.size()){
					int DRTnumber = TestPosition.get(index)+4;
				
					for(int row=3;row<=6;row++){
						value = driver.findElement(By.xpath("//*[@id='content']/div/div/div/div[3]/div["+DRTnumber+"]/div[1]/div[2]/table/tbody/tr["+row+"]/td[2]")).getText();
